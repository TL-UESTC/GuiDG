...
def get_checkpoint_path(args, name):
    root = args.output
    return os.path.join(root, name + ".pth")


@functools.lru_cache()
def create_logger(output_dir, name='123', file='1'):
    # create logger
    logger = logging.getLogger(name)
    ...
    # create file handlers
    file_handler = logging.FileHandler(os.path.join(output_dir, f'{file}.txt'), mode='w')
