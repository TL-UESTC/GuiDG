...
def get_domainbed_datasets(dataset_name, root, targets, holdout=0.2, open_ratio=0):
    ...
    if open_ratio > 0:
        ...
        for env_i, env in enumerate(datasets):
            out, in_ = misc.split_dataset(env, int(len(env) * holdout), misc.seed_hash(0, env_i))
    ...
    elif isinstance(holdout, dict):   # pre-divided 
        trains, tests = [], []
        for env_i, env in enumerate(datasets):
            if env_i not in targets:    # source training data
                in_ = misc._SplitDataset(env, holdout[env_i])
                trains.append(in_)
            else:  
                out, in_ = misc.split_dataset(env, int(len(env) * 0.2), misc.seed_hash(0, env_i))
                tests.append(out)  
        return trains, tests, class_names
    else:
